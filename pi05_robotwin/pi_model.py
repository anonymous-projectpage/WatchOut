...
class PI0:
    ...
    def get_action(self):
        ...
        _fx = os.environ.get("CS_FIXRNG", "")
        if _fx and not getattr(self.policy, "_rng_fixed", False):
            import jax as _j
            self.policy._rng = _j.random.PRNGKey(int(_fx))
            self.policy._rng_fixed = True
            print(f"[rng] fixed to {_fx}", flush=True)
